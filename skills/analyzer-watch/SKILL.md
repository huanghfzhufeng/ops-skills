---
name: analyzer-watch
description: 监听 TikTok Analyzer（自建账号数据后台）的爆款视频，「播放破 500」或「ER 破 5%」的视频立刻推送到飞书群（带视频封面的小卡片：账号 + 封面 + 播放 + ER + 发布多久 + 互动 + 看视频，整卡点击跳转，一条一卡；ER 可配最低播放门槛去噪；封面失败自动降级纯文字卡）。JWT 登录拉 /api/metrics/trending 全量视频，video_id 集合 diff 做增量去重，首次跑建 baseline 不推历史。触发：用户说「爆款预警」「analyzer 推送」「analyzer-watch」「跑一次爆款监控」「破 500 推送」，或被 /schedule 高频定时触发（默认每 10 分钟，破阈值即推）。
---
