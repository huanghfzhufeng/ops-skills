...
| **tk-niche-scout** | 给一个赛道产 **1000+ 条爆款真链接**清单（2026·≤15s·≥100万），三渠道找号 + yt-dlp 扫主页 + 四道净化 + UGC/大号分桶，内置搞笑/美妆/穿搭词典 | 「跑一个 X 赛道链接清单」 |
...
给一个赛道（搞笑/美妆/穿搭/任意）产出 **1000+ 条爆款真链接清单**，UGC 可仿拍模板置顶，交付 TSV 到 `~/Downloads/`。

```
触发：跟 Claude 说「跑一个 X 赛道链接清单」（开工前 Claude 会先做对齐四问）
```

- 硬标准写死：2026 年发布 · ≤15 秒 · 播放 ≥100 万 · yt-dlp 实测真链接 · 去重
- 三渠道找号：yaml 种子 + 榜单站现薅 + 60 个赛道搜索词雪球素人
- 四道净化：跨赛道去重（`--dedup-against`）→ 硬广 → 账号名规则 → caption 信号打分，被踢的带原因单独存档
- 分桶：粉丝 ≤50 万 = UGC模板桶置顶（按播放/粉丝比排序），大号沉底
- 内置 `niches/{comedy,beauty,fashion}.yaml`（2026-06-12 实战交付 4400 条的原版词典）；新赛道复制一份 yaml 改词典即可
- 实操细节 / 踩坑库 / 交付话术见 `skills/tk-niche-scout/SKILL.md`

### 已知限制

- 赛道纯度 ~90-95%（来源保纯 + 文字信号，不看视频；100% 需人眼）
- 硬广过滤只能抓**明示广告**（#ad/折扣码话术），未打标签的软性带货判不出来
- TikTok 搜索页只能单线程温和抓（并发轰炸 = 风控按死，详见 SKILL.md 踩坑库）
